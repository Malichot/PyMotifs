...
def annotate(path_txt, save_path):
    ...
    def read_txts(path):
        ...
        for filename in filter(
            lambda p: p.endswith("txt"), os.listdir(path)
        ):  # filtre les fichiers qui se terminent par txt
    ...
    annotated_datas["words"] = df["text"].apply(
        lambda x: nlp.tokenizer(str(x))
    )
    ...
    for doc in nlp.pipe(
        annotated_datas["words"].astype("unicode").values, batch_size=50
    ):
